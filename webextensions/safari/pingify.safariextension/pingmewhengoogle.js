...
(function() {
    ...
    var searchBox = document.querySelectorAll('[name=q]');
    ...
    httpPostAsync("https://api.pingmewhen.com/api/v1/main/submitsearch", {query: searchBoxValue},
        (r) => {
            ...
            var nav = document.querySelectorAll("[id=sfcnt]");
            if (nav.length <= 0)
                return;
            nav = nav[0];

            var pmw = document.createElement('div');
            pmw.style = "padding-left: 150px; margin-bottom: 21px; padding-top: 5px; padding-bottom: 5px; background: #fafafa; border-bottom: 1px solid #ebebeb;";
