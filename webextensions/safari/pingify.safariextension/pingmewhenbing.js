...
(function() {
    ...
    var searchBox = document.querySelectorAll('[id=sb_form_q]');
    ...
    httpPostAsync("https://api.pingmewhen.com/api/v1/main/submitsearch", {query: searchBoxValue},
        (r) => {
            ...
            var nav = document.querySelectorAll("[id=sb_form]");
            if (nav.length <= 0)
                return;
            nav = nav[0];

            var pmw = document.createElement('div');
            pmw.style = "padding-left: 100px; padding-top: 10px; padding-bottom: 5px;";
